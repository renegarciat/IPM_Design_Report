...
\\\textbf{Winding factor ($k_1$):} 0.91, a generic two-layer winding factor, using reference %TODO add a 10/12p reference paper /book.
\\\textbf{Air-gap efficiency and power factor ($\eta_{gap}$, $\cos\theta_{gap}$):} 0.95 and 0.90, generic estimates.
...
\begin{enumerate}
    ...
    \item \textbf{No-load saturation model} (Section II and III, eqs. 3--7, 38--42): air-gap flux, Carter factor and bridge leakage, giving the no-load flux density $B_{g,1o}$ (Section \ref{sec:stage2_saturation}).
    \item \textbf{Torque sizing and reaction coefficients} (Section V, eqs. 57--64, 96--110): the alignment/reluctance torque split, its geometric reaction coefficients (Section \ref{sec:torque_sizing_reaction_coefficients}), and the optimal current phase advance (Section \ref{sec:saturation_phase_advance}).
    \item \textbf{Stator winding and core sizing} (Section VI, eqs. 65--94): slot count, conductor count/gauge, tooth/yoke dimensions from the corner-point EMF and current density (Section \ref{sec:stator_winding_sizing}).
    \item \textbf{Electrical parameters} (Section VII, eqs. 95--120): $L_d$, $L_q$, $\Psi_{PM}$ (Section \ref{sec:electrical_parameters}).
\end{enumerate}
...
\end{equation}
where $\Delta$ is the linear current density, $c_d$ and $\sigma_{an}$ are geometric reaction coefficients from Stage 3 (Section \ref{sec:torque_sizing_reaction_coefficients}), $\lambda_{is}$ is the specific unsaturated reaction permeance (eq. 51, \emph{not} the stator leakage inductance -- that is a separate quantity, $L_\ell$, from paper Section VII.C, eqs. 111--119, which this pipeline does not yet compute), $k_w$ the winding factor, and $\eta_\phi$/$\sigma_s$ the PM-flux and iron saturation factors from stage 2. Maximizing Eq. \ref{eq:ricca_torque_pu} over $\gamma$ gives the optimal phase advance $\gamma_{opt}$ used in Section \ref{sec:saturation_phase_advance}.
...
\end{figure}
Stage 1 turns the stator bore and five chosen shape ratios into the rotor's V-cavity dimensions. \textbf{We choose Formula 1's bore $D_{is}=52.15\,\text{mm}$} and air gap $g=1\,\text{mm}$, the rotor outer diameter, pole pitch  and stator slot pitch follow:
\begin{align*}
...
Figure~\ref{fig:rotor_pole_geometry_scale} shows the resulting cavity to scale, at this design's own converged values, with every quantity defined above labeled directly on the geometry.
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/rotor_pole_geometry.png}
    \caption{One rotor pole's V-cavity cross-section, to scale (all lengths in mm), generated from this design's own converged Stage-1 values by \texttt{Figure\_Scripts/rotor\_pole\_geometry.py}. Broken view: the drawing is cropped just below $D_{ir}$, since the solid core continues inward to the shaft. Pole-shoe/bridge corner fillets are simplified to straight joins; every labeled dimension is exact.}
    \label{fig:rotor_pole_geometry_scale}
\end{figure}

\subsubsection*{Stage 2: No-load Saturation Model}
\label{sec:stage2_saturation}
Stage 2 (\texttt{IPMRotorSizer.computeSaturationModel\_}) turns the Stage 1 geometry and the stator slot opening into two results the rest of the procedure depends on: Carter's factor $k_C$ and the iron-saturation function $\sigma_{sM}(M_q)$ that Section \ref{sec:saturation_phase_advance} evaluates at the corner-point MMF, and the no-load fundamental air-gap flux density $B_{g,1o}$ that anchors the specific-torque equation (Eq.~\ref{eq:specific_torque}) and everything downstream of it.

\paragraph{Carter's factor.} With the slot opening $b_{as}=2\,\text{mm}$ (\texttt{spec.SlotOpening\_mm}, default), air gap $g=1\,\text{mm}$, and stator slot pitch $\tau_s=13.65\,\text{mm}$ (Section \ref{sec:rotor_geometry}), the standard slot-opening Carter coefficient (not itself one of the paper's numbered equations) gives:
\begin{equation}
...
\paragraph{Iron saturation function (eqs. 3--7).} The same stage builds the q-axis saturation function $\sigma_{sM}(M_q)$ used at the loaded corner point (Section \ref{sec:saturation_phase_advance}). It works back from the digitized M235-35A $B$--$H$ curve (Fig.~\ref{fig:bh_curve}): the tooth flux density $B_{tG}$ is solved implicitly from the air-gap flux density $B_{gI}$ (eq. 3), which sets the saturation ratio $\rho_{sat}(B_{gI})=1+H_{fe}(B_{tG})\,\rho_{hte,g}/(B_{gI}/\mu_0/k_C)$ (eq. 4) and the peak MMF $M_I(B_{gI})=B_{gI}\,g\,k_C\,\rho_{sat}/\mu_0$ needed to drive it (eq. 5). Inverting $M_I(B_{gI})$ for a given q-axis MMF $M_q$ (eq. 6) gives $\sigma_{sM}(M_q)=B_p(M_q)\,g\,k_C/(\mu_0 M_q)$ (eq. 7), plotted in Fig.~\ref{fig:saturation_factor}. The geometric ratios feeding this, $\rho_{bt,s}=b_{ts}/\tau_s$ and $\rho_{hte,g}=h_{te}/g$ (eqs. 86, 91), are supplied by Stage 4's stator winding/core sizing and updated over \texttt{solve()}'s outer iteration; at this design's converged point they settle at $\rho_{bt,s}\approx0.560$ and $\rho_{hte,g}\approx32.0$. Stage 2 itself only needs $\sigma_{sM}$ at $M_q=0$, where it is unsaturated by definition ($\sigma_{sM}\to1$) -- which is why the no-load flux below is derived independently, straight from the PM's own magnetic circuit, rather than by evaluating this function at zero.

\paragraph{No-load flux density (eqs. 24, 38--42).} At no load the PM alone drives the pole-shoe flux. The residual flux crossing the two magnet segments per pole, per unit stack length, is (eq. 24):
\begin{equation}
    \label{eq:phi_pm}
    \phi_{PM} = B_r \cdot 2w_m = 1.206\times2\times4.97\times10^{-3} \approx 11.98\,\text{mWb/m}
\end{equation}
using the temperature-derated $B_r=1.206\,\text{T}$ (Eq.~\ref{eq:br_derating}) and the magnet width $w_m=4.97\,\text{mm}$ from Stage 1 (Eq.~\ref{eq:wm}). Part of this leaks locally through the inner/outer bridges instead of crossing the air gap; \texttt{IPMRotorSizer} models both bridges combined with a single constant leakage fraction, $\ell_{no\text{-}load}=16.6\%$ (\texttt{leakage\_no\_load}), calibrated to reproduce the reference paper's own Fig.~8 at $M_q=0$. As flagged in Section \ref{sec:ricca_methodology} and revisited in Section \ref{sec:sim_framework}, this constant is one of the report's open items: calibrated to the paper's larger pole pitch, it is documented to underestimate leakage by roughly 4--6$\times$ at this project's tighter 10-pole/12-slot geometry, and is the most likely single cause of the flux-linkage shortfall traced throughout Part A.

The specific flux actually reaching the pole shoe (eq. 39), and the resulting flux density averaged over the shoe's angular span $\alpha_m\tau_r$ (eq. 40), follow as:
\begin{equation}
    \label{eq:phi_go}
    \phi_{go} = \phi_{PM}\left(1-\ell_{no\text{-}load}\right)\alpha_m = 11.98\times(1-0.166)\times0.80 \approx 7.996\,\text{mWb/m}
\end{equation}
\begin{equation}
    \label{eq:b_go}
    B_{go} = \frac{\phi_{go}}{\alpha_m\tau_r} = \frac{7.996\times10^{-3}}{0.80\times15.76\times10^{-3}} \approx 0.634\,\text{T}
\end{equation}
where $\tau_r=15.76\,\text{mm}$ is the rotor pole pitch (Section \ref{sec:rotor_geometry}). $B_{go}$ is the (roughly rectangular) flux density averaged over the pole-shoe arc; its fundamental Fourier component -- the quantity every later stage actually uses -- follows the standard width-$\alpha_m\pi$ square-wave factor (eq. 41):
\begin{equation}
    \label{eq:bg1o}
    B_{g,1o} = \frac{4}{\pi}\sin\!\left(\alpha_m\frac{\pi}{2}\right)B_{go} = \frac{4}{\pi}\sin(72^\circ)\times0.634 \approx 0.768\,\text{T}
\end{equation}
and, converting back to a specific flux for use alongside $\phi_{go}$ elsewhere in the procedure (eq. 42):
\begin{equation}
    \label{eq:phi_g1o}
    \phi_{g1o} = \frac{2}{\pi}B_{g,1o}\,\tau_r \approx 7.71\,\text{mWb/m}
\end{equation}
$B_{g,1o}=0.768\,\text{T}$ (\texttt{IPMRotorSizer.Bg1o\_T}) is the value quoted throughout the rest of Part A: the specific-torque equation (Eq.~\ref{eq:specific_torque}), the optimal phase advance search (Section \ref{sec:saturation_phase_advance}), and, downstream of the PM flux linkage $\Psi_{PM1}$, the corner-speed check of Section \ref{sec:corner_speed}.
...
\end{equation}
computed alongside $\gamma_{opt}$ in \texttt{computeTorqueSizing\_} (Section \ref{sec:saturation_phase_advance}).
...
\end{equation}
where $h_{ob}=(\tau_r-2w_{hr}-b_{ps})/2=0.21\,\text{mm}$ (eq. 9, not previously shown in Section \ref{sec:vshape_definition}) is the rotor-OD arc spanned by the outer bridge -- small here because this design's wide pole shoe ($\alpha_m=0.80$) leaves little room between adjacent shoes -- and $p=5$ pole pairs converts that mechanical arc into electrical radians. Beyond $\theta_{e,rib}$, out to $\pi/2$, the rotor is treated as isotropic (unslotted, no shoe).
...
\end{equation}
and the unsaturated anisotropy ratio used directly in Eq.~\ref{eq:ricca_torque_pu} follows:
\begin{equation}
\label{eq:sigma_an}
\sigma_{an} = \sigma_{an,o} = \frac{c_q}{c_d} = \frac{0.951}{0.322} \approx 2.95 \quad\text{(eq. 110)}
\end{equation}

These closed forms were checked against the paper's own worked example (\texttt{test\_IPMRotorSizer.m}, evaluated at its Table I geometry: $\alpha_m=0.754$, $D=160\,\text{mm}$, 8 poles) and reproduce its reference values $c_d=0.201$, $c_q=0.825$, $\sigma_{an,o}=4.11$ to within 2\%. This project's own values differ substantially -- $c_d=0.322$ vs.\ $0.201$, $\sigma_{an}=2.95$ vs.\ $4.11$ -- mainly because the wider pole shoe chosen here ($\alpha_m=0.80$ vs.\ $0.754$) and the much smaller bore push $\theta_{e,rib}$ closer to $\theta_{ps}$ (a $2.4^\circ$ rib slice out of a $90^\circ$ quarter-pole here, versus a proportionally larger one in the paper's own design), shrinking the ``blocked'' rib share relative to the isotropic tail and letting more of the reaction MVD reach the air gap directly -- particularly on the d-axis, where the internally-absorbed $U_{psd}$ term is the main thing keeping $c_d$ low.

With $c_d$, $\sigma_{an}$, and $\lambda_{is}$ fixed by geometry alone, that is everything Eq.~\ref{eq:ricca_torque_pu} needs from the reaction side; combining them with Stage 2's saturation factors ($\eta_\phi$, $\sigma_s$) to actually maximize $f_T$ over $\gamma$ and size the stack length is carried out next, in Section~\ref{sec:saturation_phase_advance}. One subtlety worth flagging: because \texttt{computeTorqueSizing\_} (which evaluates $\gamma_{opt}$) runs \emph{before} \texttt{computeElectricalParameters\_} within a given pass of \texttt{solve()}, the $c_d$/$\sigma_{an}$ values it actually uses are the \emph{previous} iteration's converged result -- seeded from the paper's own Table I defaults ($c_d=0.201$, $\sigma_{an,o}=4.11$) on the very first pass -- which is itself part of why the five-stage loop needs more than one iteration to begin with (Section~\ref{sec:ricca_methodology}); for this design point, \texttt{IPMRotorSizer.solve()} converges in 2 iterations.
...
With the corner-point flux and current-density targets fixed, this stage sizes the winding turns, wire gauge, and stator tooth/slot/yoke dimensions (paper Section VI, eqs. 65--94; \texttt{IPMRotorSizer.\allowbreak computeStatorWindingSizing\_}) -- the source of every stator-side number already quoted in Table~\ref{tab:params_calculated}, not previously derived.

The fundamental pole flux at the corner point, and the EMF per conductor it induces, follow from Stage 2's no-load flux $\phi_{g1o}=7.71\,\text{mWb/m}$ and Stage 3's $\eta_{\phi,c}=0.983$, at the corner frequency $f_c=1020\,\text{Hz}$:
\begin{equation}
\label{eq:phi_g1c}
\Phi_{g1c} = \eta_{\phi,c}\,\phi_{g1o}\,l_e = 0.353\,\text{mWb}, \qquad E_{cc} = \frac{\pi}{\sqrt2}f_c\,\Phi_{g1c} = 0.80\,\text{V} \quad\text{(eqs. 65--66)}
\end{equation}
Targeting an EMF-over-voltage utilization $\rho_{EV}=0.65$ against the maximum inverter phase voltage $V_{invM}=0.95V_{dc}/(2\sqrt2)=173.0\,\text{V}$ (eq. 68 -- a different, more conservative voltage-limit model than the modulation-index one used in Section~\ref{sec:corner_speed}; the two haven't been reconciled) gives the conductors in series, rounded through an even conductors-per-slot count:
\begin{equation}
\label{eq:conductors_series}
U_{c,th} = \frac{\rho_{EV}V_{invM}}{k_w E_{cc}} = 154.6, \qquad u = 2\,\text{round}\!\left(\tfrac12\,\frac{3U_{c,th}(P/2)}{N_s}\right) = 194, \qquad U_c = \frac{N_s u}{3(P/2)} = 155.2
\end{equation}
(eqs. 69, 72, 73). $U_c$ feeds Stage 5's inductances directly. Sizing the wire gauge from the current-density target $J_{s,rms}=8\,\text{A/mm}^2$, then the slot area from the copper fill factor $k_{Cu}=0.40$ (eqs. 76--84), fixes the tooth width and stator yoke height against the flux-density targets $B_{ts}=1.415\,\text{T}$, $B_{cs}=1.00\,\text{T}$:
\begin{equation}
\label{eq:tooth_yoke}
b_{ts} = \frac{B_{g,1o}}{B_{ts}}\frac{\tau_s}{k_{st}} = 7.64\,\text{mm} \quad\text{(eq. 85)}, \qquad h_{sy} = \frac{\phi_{g1o}}{2B_{cs}k_{st}} = 3.97\,\text{mm} \quad\text{(eq. 93)}
\end{equation}
using $\tau_s=13.65\,\text{mm}$ (Section~\ref{sec:vshape_definition}). Adding the resulting slot height ($h=22.9\,\text{mm}$, the closed-form root of a trapezoidal slot-area equation, eq. 89) and the slot-opening height $h_{as}=1.5\,\text{mm}$ (class default) to the bore gives the external stator diameter:
\begin{equation}
\label{eq:stator_od}
D_{es} = D_{is} + 2(h_{as}+h+h_{sy}) = 52.15 + 2(1.5+22.9+3.97) \approx 108.8\,\text{mm} \quad\text{(eq. 94)}
\end{equation}
matching Table~\ref{tab:params_calculated}'s stator diameter exactly. One open caveat: this same current-density/linear-current-density pairing also fixes a phase current $I_c=31.7\,\text{A}_{rms}$ (eq. 75) that is not what the rest of Part A or the FEM pipeline actually use -- a third figure alongside the $I_{pk}=74.2\,\text{A}$ (Eq.~\ref{eq:iq_current}) and $I_{peak}\approx75.8\,\text{A}$ (Section~\ref{sec:part_b_fea}) already flagged there, tracing to the same tension between the assumed linear current density $\Delta$ and the actual flux-linkage-limited torque point.

\subsubsection*{Stage 5: Electrical Parameters}
\label{sec:electrical_parameters}
The last stage turns Stage 3's reaction coefficients and Stage 4's conductor count into the actual $d$/$q$ inductances and PM flux linkage (paper Section VII, eqs. 48, 52--56; \texttt{IPMRotorSizer.\allowbreak computeElectricalParameters\_} -- the same method that computes $c_d$, $c_q$, Section~\ref{sec:torque_sizing_reaction_coefficients}). The unsaturated reaction inductances follow directly from $\lambda_{is}$ (Eq.~\ref{eq:lambda_is}) and $U_c$ (Eq.~\ref{eq:conductors_series}):
\begin{equation}
\label{eq:ldo_lqo}
L_{do} = c_d\,\lambda_{is}\,\frac{U_c^2}{P}\,l_e = 0.172\,\text{mH}, \qquad L_{qo} = c_q\,\lambda_{is}\,\frac{U_c^2}{P}\,l_e = 0.509\,\text{mH} \quad\text{(eq. 52)}
\end{equation}
...
\end{equation}
The PM flux linkage, already used without derivation in Eq.~\ref{eq:iq_current} and Section~\ref{sec:corner_speed}, follows the same pattern (eq. 48):
\begin{equation}
\label{eq:psipm1}
\Psi_{PM1} = \frac{k_w U_c \phi_{g1o}\,\eta_{\phi,c}}{2\sqrt2}\,l_e = \frac{0.91\times155.2\times0.00771\,\text{Wb/m}\times0.983}{2\sqrt2}\times0.04656\,\text{m} \approx 0.0176\,\text{Wb}
\end{equation}
Not computed by this pipeline: the true phase leakage inductance $L_\ell$ and AC winding resistance $R(f)$ (paper Section VII.C, eqs. 111--119) -- already flagged in passing at Section~\ref{sec:torque_sizing_reaction_coefficients}'s $\lambda_{is}$ correction. With $L_d$, $L_q$, and $\Psi_{PM1}$ fixed, everything Part A's summary tables below actually draw on -- rotor geometry, stack length, phase advance, stator core, and rated current -- now traces back to a shown equation rather than an unexplained number.
