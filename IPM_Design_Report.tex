...
Inverting $M_I(B_{gI})$ for a given q-axis MMF $M_q$ gives $\sigma_{sM}(M_q)=B_p(M_q)\,g\,k_C/(\mu_0 M_q)$, plotted in Fig.~\ref{fig:saturation_factor}. The geometric ratios feeding this, $\rho_{bt,s}=b_{ts}/\tau_s$ and $\rho_{hte,g}=h_{te}/g$ (eqs. 86, 91), can't be pinned down here: $b_{ts}$ (tooth width) and $h_{te}$ (tooth height) are only sized in Stage 4, from the stator winding/core geometry -- but Stage 4 sizes them \emph{from} $B_{g,1o}$ (eq. 85: $b_{ts}=(B_{g,1o}/B_{ts})\tau_s/k_{st}$), which is itself an output of this saturation model. \texttt{solve()} breaks that circularity by treating $\rho_{bt,s}$ and $\rho_{hte,g}$ as 2 of its 4 iterative parameters (alongside $c_d$ and $\sigma_{an}$, Section~\ref{sec:torque_sizing_reaction_coefficients}): seeded at the paper's own Table~I values ($\rho_{bt,s}=0.704$, $\rho_{hte,g}=52.4$), re-derived by Stage 4 at the end of every outer-loop pass, and fed back into Stage 2 on the next -- repeating until all four stop moving (\texttt{solve()} converges in 2 passes for this design). At this design's converged point they settle at $\rho_{bt,s}\approx0.560$ and $\rho_{hte,g}\approx32.0$. Stage 2 itself only needs $\sigma_{sM}$ at $M_q=0$, where it is unsaturated by definition ($\sigma_{sM}\to1$) -- which is why the no-load flux above was derived independently, straight from the PM's own magnetic circuit, rather than by evaluating this function at zero.
